...
/**
 * Verificio se a variavel de ações existe, para então verificar qual ação o usuário ordenou.
 */
 if(isset($_GET['acao_carrinho'])) {
      ...
      switch($_GET['acao_carrinho']) {
        ...
        case 'add': //adiciona um produto ao carrinho.
          //Pegando as varíaveis da URL com os dados dos produtos (Isso não é o ideal, apenas didático)
          $produto = $_GET['carr_produto'];
          $preco = $_GET['carr_preco'];
          $qnt = $_GET['carr_qnt'];
          $total = $preco * $qnt;

            $dados_produtos_a_ser_add = array("prodto" => $produto, "preco" => $preco, "quantidade" => $qnt, "total" => $total);
             ...
             case 'atualizar-qnt':
              ...
              if(isset($_SESSION['carrinho'][$item_carrinho])) {//verificando se o item existe...
                ...
                $total = $_SESSION['carrinho'][$item_carrinho]['preco'] * $qnt; //atualizando o total...
                 ...
                 $_SESSION['carrinho'][$item_carrinho]['quantidade']= $qnt;//atualizando a nova quantidade.
